...
void generic_ui8vaddu(int n, const uint8_t *lhs, int lhs_stride,
                      const uint8_t *rhs, int rhs_stride, uint8_t *dst,
                      int dst_stride) {
  for (int i = 0; i < n; ++i) {
    dst[i] = lhs[i] + rhs[i];
  }
...
TEST(ivlib, ivadd_aligned_memory) {
  ...
  ui8vadd(N, lhs, N, rhs, N, sum, N);

  uint8_t expect[N];
  generic_ui8vaddu(N, lhs, N, rhs, N, expect, N);
...
TEST(ivlib, ui8vaddu) {
  ...
  ui8vaddu(N - 3, lhs + 3, N, rhs + 3, N, actual + 1, N);

  uint8_t expect[N] = {33};
  generic_ui8vaddu(N - 3, lhs + 3, N, rhs + 3, N, expect + 1, N);
